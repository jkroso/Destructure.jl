...
@destruct [:a=>a] = Dict(:a=>1)
@test a == 1
@destruct [:a=>a,tail...] = Dict(:a=>0,:b=>1,:c=>2)
...
@destruct [:a=>[:b=>child],tail...] = Dict(:a=>Dict(:b=>1))
@test child == 1 && tail == Dict()

@destruct [[a],[b],tail...] = [[1],[2]]
@test a == 1 && b == 2 && tail == []

@destruct [[:a=>a],[b],tail...] = [Dict(:a=>0),[1]]
@test a == 0 && b == 1 && tail == []

@destruct [:num=>num,:den=>den, tail...] = 1//2
@test num == 1 && den == 2 && tail == Dict()

@destruct [:a=>a=1] = Dict()
@test a == 1

@destruct destruct([:a=>a]::Dict,b) = (a,b)
@test destruct(Dict(:a=>1),2) == (1,2)

@const [:d=>d, dtail..., :f=>[f]] = Dict(:d=>1,:e=>2,:f=>[3])
